    	...
    	try(Connection conn=dbcs.getConnection();//获取数据库接
    		...
    		for(int i=1;i<=1000;) {
    			...
    			if(!alist.contains(id)) {//判断学号是否唯一
    				...
    				String fzr=getSex();
    				String jf=id;
    				String xmm=getChineseName(fzr);//随机获取姓名
    				int cj=getNum(0, 100);//随机获取成绩
    				pstmt.setString(1, xmm);//定义第1个占位符的内容
    				pstmt.setString(2, id);//定义第2个占位符的内容
    				pstmt.setString(3, jf);//定义第3个占位符的内容
    	    		pstmt.setString(4, fzr);//定义第4个占位符的内容
